...
static void	ft_def_spr_pos(t_cub3d *cub3d, int i)
{
	...
	cub3d->sprite.x
		= cub3d->spr_crds[cub3d->sprite.sprite_order[i]].x
		- cub3d->player.pos_x;
	cub3d->sprite.y
		= cub3d->spr_crds[cub3d->sprite.sprite_order[i]].y
		- cub3d->player.pos_y;
	cub3d->sprite.inv_det
		= 1.0 / (plane_x * dir_y - cub3d->move.plane_y * dir_x);
...
static void 	ft_def_spr_size(t_cub3d *cub3d)
{
	...
	cub3d->sprite.start_draw_x
		= -cub3d->sprite.widht / 2 + cub3d->sprite.sprite_screen;
	if (cub3d->sprite.start_draw_x < 0)
		cub3d->sprite.start_draw_x = 0;
	cub3d->sprite.end_draw_x
		= cub3d->sprite.widht / 2 + cub3d->sprite.sprite_screen;
...
static void 	ft_spr_draw(t_cub3d *cub3d, int x, int i)
{
	...
	while (y < cub3d->sprite.end_draw_y)
	{
		...
		cub3d->sprite.sprite_text_y
			= ((d * cub3d->texture[6].height) / cub3d->sprite.height) / 256;
		get_color(cub3d, 6, cub3d->sprite.sprite_text_x,
			cub3d->sprite.sprite_text_y);
